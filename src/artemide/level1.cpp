...
static uint8_t obstacle[STRIP_TILES];
static uint8_t wireAnchor[STRIP_TILES]; // 1 = якорь провода
static uint8_t ceiling[STRIP_TILES]; // 1 = блок потолка (2 тайла на провод)
static uint8_t coin[STRIP_TILES]; // 0 = пусто, 1 = плюс, 2 = ноль
...
static void generateTile() {
  ...
  // МОНЕТА
  coin[idx] = 0;
  if ((obstacle[idx] == 0) && (wireAnchor[idx] == 0) && (ceiling[idx] == 0) && (coinCooldown == 0)) {
    bool nearObs = false;
    if (segObsAt != 255) {
      uint8_t d;
      if (segPos >= segObsAt) {
...
static uint8_t groundAt(int16_t wx) {
  if (wx < 0) {
    return 48;
  }
  int idx = (wx / TILE_SIZE) % STRIP_TILES;
  if (idx < 0) {
    idx += STRIP_TILES;
  }
  return groundTop[idx];
}
...
void level1Update(hero &h) {
  ...
  int16_t needUntil = (worldX / TILE_SIZE) + STRIP_TILES - 4;
  ...
  bool hasCeiling = (ceiling[feetTile] != 0);
  ...
  if (h.score >= 30) {//увеличить до 255 после тестировки
    state = 1;
  }
}
