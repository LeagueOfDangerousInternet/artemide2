...
void level1Update(hero &h) {
  ...
  // IMPROVEMENT: finish at the same 25-point target shown by both HUDs.
  if (h.score >= LEVEL1_TARGET) {
    state = 1;
  }
}
...
void level1Draw(hero &h) {
  // IMPROVEMENT: a sparse scrolling star field connects level 1 visually to
  // level 2 while the procedural platforms remain unique to this level.
  for (uint8_t i = 0; i < 8; ++i) {
    int16_t starX = (int16_t)((i * 23 + 128 - (camX / 4) % 128) % 128);
    arduboy.drawPixel(starX, 10 + (i * 11) % 24, WHITE);
  }

  // IMPROVEMENT: speed streaks appear only during strong Right acceleration.
  if (h.vx >= 2) {
    for (uint8_t i = 0; i < 4; ++i) {
      int16_t y = 14 + i * 9;
      arduboy.drawFastHLine((time * 7 + i * 29) % 96, y, 7, WHITE);
    }
  }
  ...
  // IMPROVEMENT: display the real target, producing 25/25 at completion.
  arduboy.print(F("/"));
  arduboy.print(LEVEL1_TARGET);
